...
static void adicionaProcessoNaLista (pLeitor pleitor, pProcessoInterpretador pprocesso);
static void retiraProcessoDaLista (pLeitor pleitor, pProcessoInterpretador processo);
...
typedef struct _leitor{
	...
	char** vetComandos;
	int comandoAtual;
	pProcessoInterpretador lisProcesso;
	pProcessoInterpretador proximoProcesso;
} Leitor;


pProcessoInterpretador criarProcesso ()
{
	pProcessoInterpretador processoRet = (pProcessoInterpretador) malloc(sizeof(ProcessoInterpretador));
	if(processoRet != NULL)
		return processoRet;
...
{
	pProcessoInterpretador pprocesso;
	int numeroLinhas = 0;
	int linhaAtual = 0;
	char * nomeDoProcesso;
...
pProcessoInterpretador iterarComando (pLeitor pleitor)
{
	 ...
	 pProcessoInterpretador aux;

	 if (pleitor->comandoAtual >= pleitor->qtdComandos)
	  return NULL;

	 aux = pleitor->lisProcesso;
	 retiraProcessoDaLista (pleitor, pleitor->lisProcesso);
	 pleitor->qtdComandos--;
...
int getQtdComando(pLeitor pleitor)
{
 ...
 void retiraProcessoDaLista (pLeitor pleitor, pProcessoInterpretador processo)
{
	pProcessoInterpretador aux, anterior = NULL;
 ...
 char * pegaNomeDoProcesso (char* string)
{
 ...
 void adicionaProcessoNaLista (pLeitor pleitor, pProcessoInterpretador pprocesso)
{
	pProcessoInterpretador aux = pleitor->lisProcesso;
